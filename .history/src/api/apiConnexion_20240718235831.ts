...
import { CategorieListeResponse, ListeCategorie } from "../models/categorie/getListeCategorieResponse";
...
const apiConnexion = {
    ...
    getAllCategories: async (): Promise<CategorieDTO[]> => {
        try {
            const response: AxiosResponse<CategorieListeResponse> = await axios.get(`${API_BASE_URL}/liste`);
