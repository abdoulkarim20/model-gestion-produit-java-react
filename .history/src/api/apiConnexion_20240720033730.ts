...
const apiConnexion = {
    ...
    createCategorie: async (categorie: CategorieDTO): Promise<CategorieDTO> => {
        try {
            const response = await axios.post<CategorieDTO>(`${API_BASE_URL}/categories/save-categorie`, categorie);
    ...
    updateCategorie: async (categorie: CategorieDTO): Promise<CategorieDTO> => {
        try {
            const response = await axios.put<CategorieDTO>(`${API_BASE_URL}/categories/update-categorie/${categorie.id}`, categorie);
            return response.data;
        } catch (error) {
    ...
    deleteCategorie: async (id: number): Promise<void> => {
        try {
            await axios.delete<number>(`${API_BASE_URL}/categories/delete-categorie/${id}`)
        } catch (error) {
            console.log("Erreur lors de la suppression d'une categorie", error);
            throw error;
        }
    },
    getCategorieDetail: async (id: string): Promise<CategorieDTO | undefined> => {
        try {
            const response = await axios.get<CategorieDTO>(`${API_BASE_URL}/categories/detail-categorie/${id}`)
            return response.data;
        } catch (error) {
